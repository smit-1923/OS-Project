...
{
    let validInputs = ["fcfs", "sstf", "scan", "cscan","look","clook"];
...
function look(x, y, xAxisStart, points, multiplier){
    ...
    headMovementCount=(a[a.length-1]-lastValue)+(a[a.length-1]-b[b.length-1]);
...
function clook(x, y, xAxisStart, points, multiplier){
    ...
    headMovementCount=(a[a.length-1]-lastValue)+(a[a.length-1]-b[0])+(b[b.length-1]-b[0]);
    let flag=0;
    for(let i=0;i<a.length;i++){
        x = xAxisStart+a[i]*multiplier;
        drawPoint(x, y, 4);
    ...
    for(let i=0;i<b.length;i++){
        ...
        drawPoint(x, y, 4);
        if(flag==0){
            drawDottedLine(prevPointXValue, y-30, x, y);
            flag=1;
        }
        else
            drawLine(prevPointXValue, y-30, x, y);
